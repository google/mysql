                 ...
                 bool keep_row_order)
{
  ...
  */
  ...
  {
    my_snprintf(path, sizeof(path), "%s_select_%lx_%i", tmp_file_prefix,
                current_pid, temp_pool_slot);
  }
  else
  {
    /* if we run out of slots or we are not using temp_pool */
    my_snprintf(path, sizeof(path), "%s_select_%lx_%lx_%x", tmp_file_prefix,
...
int JOIN::save_explain_data_intern(Explain_query *output, bool need_tmp_table,
                                   ...
                                   const char *message)
{
  Explain_node *explain_node= NULL;
