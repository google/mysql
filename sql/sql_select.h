...
typedef struct st_join_table {
  ...
  */ 
  ...
  enum {
    /* If set, the rowid of this table must be put into the temp table. */
    KEEP_ROWID= 1,

    /*
      If set, once should call h->position() to obtain rowid,
      otherwise, the rowid is assumed to already be in h->ref
      (this is because join caching and filesort() save the rowid and
      then put it back into h->ref)
    */
    CALL_POSITION= 2
  };
...
protected:
  ...
  /* Workaround a change of API in SQL_SELECT::skip_record */
  inline bool select_skip_record(THD *thd, SQL_SELECT *select) {
    bool skip_record;
    bool error = select->skip_record(thd, &skip_record);
    return !error && skip_record;
  }
...
// true iff thd->lex->select_lex is safe for use as part of an INSERT or
// UPDATE...SELECT...LIMIT query.
