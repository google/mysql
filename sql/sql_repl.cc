...
/*
  ...
  exist in earlier binlogs. Also if ok the format description event read from
  the binlog is returned in *out_fdle.
*/
static const char *
get_gtid_list_event(IO_CACHE *cache,
                    Gtid_list_log_event **out_gtid_list,
                    Format_description_log_event **out_fdle)
{
  ...
  *out_fdle= static_cast<Format_description_log_event *>(ev);

  for (;;)
  {
    Log_event_type typ;

    ev= Log_event::read_log_event(cache, 0, *out_fdle,
                                  opt_master_verify_checksum);
    if (!ev)
    {
...
// Meta information about GTID index file.
//
// The main purpose of this data structure is to have a mutex that will allow
// to do only one index regeneration per binlog file at a time even when
// several connections from slaves detected index corruption and wanted to
// regenerate it. By avoiding this contention we avoid excessive I/O due to
// the same binlog file scanned from many threads at once, which also makes
// one regeneration faster.
// This structure is not permanently kept in memory for each existing GTID
// index file. It lives only as long as some slave uses an index to find
// binlog position. The necessity of still keeping the structure in memory is
// detected by reference counting.
struct GTID_INDEX_META
{
  char file_name[FN_REFLEN];
  size_t file_name_len;
  mysql_mutex_t regen_lock;
  uint ref_cnt;
  bool regenerated;
};

// All active GTID indexes meta info hashed by the index file name.
static HASH gtid_indexes;
// Mutex to access gtid_indexes.
static mysql_mutex_t gtid_index_meta_lock;


// Extract file name from GTID_INDEX_META structure.
// Function is used by gtid_indexes hash.
static uchar *
get_index_name_from_meta(GTID_INDEX_META *meta, size_t *length,
                         my_bool not_used __attribute__((unused)))
{
  *length= meta->file_name_len;
  return (uchar *) meta->file_name;
}

// Delete GTID_INDEX_META structure.
// The function is called when GTID_INDEX_META instance is deleted from
// gtid_indexes.
static void
free_gtid_index_meta(void *ptr)
{
  GTID_INDEX_META *meta= (GTID_INDEX_META *)ptr;
  mysql_mutex_destroy(&meta->regen_lock);
  delete meta;
}

// Open GTID index file for reading and create GTID_INDEX_META structure if
// necessary (or return pointer to already existing one). Reference counter on
// the meta info will be increased, and it should be decreased later by call to
// release_gtid_index_meta_ref().
//
// Parameters:
//   index_name - GTID index file name
//   index_meta - pointer to variable that will contain pointer to meta info.
//       The meta info will be the same as on input if it's not NULL, or will
//       be fetched from gtid_indexes and created if necessary if the variable
//       is NULL on input.
//   out_saw_regen - output variable that will contain true if the meta info
//       had "regenerated" flag set at the time of file opening. I.e. the
//       variable will have true if further index processing will look into
//       a file that was just regenerated by another thread, which means it
//       doesn't need another regeneration if some problem with it was detected.
static File
open_gtid_index_file(const char *index_name,
                     GTID_INDEX_META **index_meta,
                     bool *out_saw_regen)
{
  mysql_mutex_lock(&gtid_index_meta_lock);
  if (!*index_meta)
  {
    GTID_INDEX_META *meta= (GTID_INDEX_META *)
                           my_hash_search(&gtid_indexes,
                                          (const uchar *) index_name,
                                          strlen(index_name));
    if (!meta)
    {
      meta= new GTID_INDEX_META;
      mysql_mutex_init(NULL, &meta->regen_lock, NULL);
      meta->file_name_len= strlen(index_name);
      strcpy(meta->file_name, index_name);
      meta->ref_cnt= 0;
      meta->regenerated= false;
      my_hash_insert(&gtid_indexes, (uchar *) meta);
    }
    ++meta->ref_cnt;
    *index_meta= meta;
  }
  File index_fd= my_open(index_name, O_RDONLY | O_BINARY, MYF(MY_WME));
  *out_saw_regen= (*index_meta)->regenerated;
  mysql_mutex_unlock(&gtid_index_meta_lock);
  return index_fd;
}

// Release reference to the GTID index meta info and delete it from memory if
// the reference was last.
static void
release_gtid_index_meta_ref(GTID_INDEX_META *meta)
{
  mysql_mutex_lock(&gtid_index_meta_lock);
  if (--meta->ref_cnt == 0)
    my_hash_delete(&gtid_indexes, (uchar *) meta);
  mysql_mutex_unlock(&gtid_index_meta_lock);
}

// Read one record from GTID index file.
//
// Parameters:
//   index_cache - open IO cache for the index file
//   index_pos - on input it's the position in the index file to start reading
//       the record from. If *index_pos points into the middle of a record then
//       the next record will be read and *index_pos will be adjusted to contain
//       actual position of a record that was read.
//   out_glev - after return will contain Gtid_list event saved in the index
//       record read
//   out_binlog_pos - after return will contain binlog position saved in the
//       index record read
//   out_gtid - after return will contain the GTID saved in the index record
//       read
//   errormsg - pointer to variable where to store pointer to error message
//       if necessary
//
// Returns:
//   false in case of successful read of a record or detection the end of index
//   file. End of index file can be detected by untouched *out_glev pointer.
//   Returns true in case of error, *errormsg will contain error message in
//   this case.
static bool
read_gtid_index_record(IO_CACHE *index_cache,
                       my_off_t *index_pos,
                       Gtid_list_log_event **out_glev,
                       my_off_t *out_binlog_pos,
                       String *out_gtid,
                       const char **errormsg)
{
  char line_buff[1024];
  my_off_t file_size= my_b_filelength(index_cache);
  char *gtid_list_ptr;
  rpl_gtid *gtid_list;
  uint32 gtid_list_len;
  size_t gtid_list_strlen;
  char *gtid_ptr;
  char *next_ptr= NULL;
  int error= 0;

  DBUG_ASSERT(*index_pos > 0);
  // If *index_pos points to the beginning of the record we need to read it,
  // not the record after it. So move backwards one byte for the following
  // double-read to work.
  --(*index_pos);
  my_b_seek(index_cache, *index_pos);
  // Assuming initial *index_pos can be in the middle of the record we skip
  // the first line and will parse the next one.
  for (int i = 0; i < 2; ++i)
  {
    *index_pos= my_b_tell(index_cache);
    if (*index_pos >= file_size)
      return false;
    if (!my_b_gets(index_cache, line_buff, sizeof(line_buff)))
    {
      *errormsg= "Cannot read a record from GTID index";
      return true;
    }
  }

  *out_binlog_pos= (my_off_t) my_strtoll10(line_buff, &next_ptr, &error);
  if ((error != 0 && error != -1) || *next_ptr != ';')
  {
    *errormsg= "Cannot read binlog position from GTID index file";
    goto bad_record;
  }

  gtid_list_ptr= ++next_ptr;
  next_ptr= strchr(gtid_list_ptr, ';');
  if (!next_ptr)
  {
    *errormsg= "Cannot find end of GTID list in the record from GTID index file";
    goto bad_record;
  }
  *next_ptr= 0;
  gtid_list_len= 0;
  gtid_list_strlen= next_ptr - gtid_list_ptr;
  gtid_list= gtid_parse_string_to_list(gtid_list_ptr, gtid_list_strlen,
                                       &gtid_list_len);
  if (!gtid_list && gtid_list_strlen != 0)
  {
    *errormsg= "Cannot parse GTID list";
    goto bad_record;
  }
  *out_glev= new Gtid_list_log_event(gtid_list, gtid_list_len, 0);
  my_free(gtid_list);
  if (!(*out_glev)->is_valid())
  {
    delete *out_glev;
    *out_glev= NULL;
  ...
  if (next_ptr)
    out_gtid->append(gtid_ptr, next_ptr - gtid_ptr);
  else
    out_gtid->append(gtid_ptr);

  return false;

bad_record:
  sql_print_error("%s: '%s'", *errormsg, line_buff);
  return true;
}

// Scan GTID index file to find a binlog position to start reading from when
// slave requests to start replication.
//
// Parameters:
//   index_name - name of the index file
//   binlog_cache - open IO_CACHE for the binlog itself; used to check that its
//       state on disk correlates with GTID index (it can be behind due to
//       write caching)
//   fdle - Format_description event to use when reading events from the binlog
//   is_binlog_active - true if the binlog file is the currently active binlog;
//       that's the only case when binlog is allowed to be behind the GTID
//       index, for non-active binlog file correlation is not checked as it
//       should be always flushed after rotation
//   state - replication starting position requested by slave
//   slave_gtid_strict_mode - value of gtid_strict_mode on the slave
//   index_meta - pointer to a variable that will contain or already contains
//       pointer to GTID_INDEX_META with meta info about the index file to be
//       scanned
//   out_gtid - on return will contain GTID value from the found index record
//   out_binlog_pos - on return will contain binlog position from the found
//       index record
//   out_glev - on return will contain Gtid_list event from the found index
//       record
//   out_saw_regen - on return will contain value of "regenerated" flag from
//       the index meta info during the time of opening the file
//   errormsg - on return will contain error message in case of error
//
// Returns:
//   false in case of successfully finding an index record, or in case if the
//   index file is empty (i.e. binlog file doesn't have any transactions). The
//   case of empty index file can be detected by emptiness of out_gtid.
//   Returns true in case of error, *errormsg contains specific error message
//   in this case.
static bool
gtid_scan_binlog_index(const char *index_name,
                       ...
                       const char **errormsg)
{
  ...
  *out_binlog_pos= 4;
  ...
  my_off_t start_pos, low_pos, high_pos;
  String state_str;

  File index_fd= open_gtid_index_file(index_name, index_meta, out_saw_regen);
  if (index_fd < 0)
  {
    *errormsg= "Cannot open the GTID index file";
    goto end;
  }
  bzero(&index_cache, sizeof(index_cache));
  if (init_io_cache(&index_cache, index_fd, IO_SIZE, READ_CACHE, 0, 0,
                    MYF(MY_WME | MY_NABP)))
  {
    *errormsg= "Cannot open the GTID index cache";
    goto end;
  }
  if (check_gtid_index_header(&index_cache, errormsg))
  {
    *errormsg= "GTID index file failed the check of the header";
    goto end;
  }

  /*
    Binary search the GTID index file for the last record that has all GTIDs
    from the requested slave state. If the scanned GTID index is on the
    currently active binlog file then also an attempt to read the GTID event
    from the binlog is made. If attempt fails then apparently some of the last
    events in the binlog were not flushed to disk yet, so we will attempt to
    move to an earlier record. That is acceptable because eventually
    mysql_binlog_send() will scan the binlog sequentially anyway, so it will
    reach the necessary position whenever it's flushed to disk.
  */
  low_pos= my_b_tell(&index_cache);
  start_pos= low_pos;
  high_pos= my_b_filelength(&index_cache);
  while (low_pos != high_pos)
  {
    delete glev;
    glev= NULL;
    my_off_t cur_pos= (low_pos + high_pos) / 2;
    out_gtid->length(0);
    if (read_gtid_index_record(&index_cache, &cur_pos, &glev,
                               out_binlog_pos, out_gtid, errormsg))
      goto end;
    if (glev && contains_all_slave_gtid(state, glev, slave_gtid_strict_mode))
    {
      if (is_binlog_active)
      {
        // Binlog file is active, so some events may be not flushed to disk yet.
        // We need to check that this position is readable, otherwise start
        // from some earlier position.
        my_b_seek(binlog_cache, *out_binlog_pos);
        Log_event *ev= Log_event::read_log_event(binlog_cache, 0, fdle,
                                                 opt_master_verify_checksum);
        bool is_event_read= ev != NULL;
        delete ev;
        if (!is_event_read)
        {
          sql_print_warning("Cannot read GTID event from binlog position "
                            "%llu (binlog file for GTID index '%s'), will try "
                            "an earlier event.",
                            (ulonglong)*out_binlog_pos, index_name);
          if (high_pos <= cur_pos)
          {
            // See the comment on why this is necessary below.
            high_pos= low_pos;
          }
          else
          {
    ...
    else if (high_pos <= cur_pos)
    {
      // Deviation from binary search algorithm. Our file positions are
      // fuzzy, so if low_pos at the beginning of the loop was pointing to one
      // record and high_pos was pointing to the next, then cur_pos here will
      // be the same as high_pos (initial calculation of cur_pos will point to
      // the middle of the record and it will be corrected in
      // read_gtid_index_record). In such case we should stop looping and just
      // proceed with the record at low_pos.
      high_pos= low_pos;
    }
    else
    {
  ...
  glev= NULL;
  out_gtid->length(0);
  state->to_string(&state_str);
  if (!read_gtid_index_record(&index_cache, &low_pos, out_glev,
                              out_binlog_pos, out_gtid, errormsg))
  {
    ret= false;
  }
  else if (is_binlog_active && low_pos == start_pos)
  {
    sql_print_warning("Cannot read first record from GTID index file %s. "
                      "Assuming the file is empty.", index_name);
    *errormsg= NULL;
    out_gtid->length(0);
    ret= false;
  }
  ...
  if (index_fd >= 0)
  {
    ...
    my_close(index_fd, MYF(MY_WME));
  }
  return ret;
}

// Find binlog position to start replication from within a binlog file.
// Scans GTID index for the binlog file, finds the position to start replicating
// from and checks that the position actually matches the contents of the
// binlog file.
//
// Parameters:
//   gtid_index_name - name of the GTID index file
//   binlog_cache - open IO cache for the binlog
//   fdle - format description even that was read from the binlog file and can
//       be used to read binlog events from it
//   state - replication position requested by slave
//   slave_gtid_strict_mode - value of gtid_strict_mode on the slave
//   index_meta - pointer to a variable that will contain or already contains
//       pointer to GTID_INDEX_META with meta info about the index file to be
//       scanned
//   out_pos - on return will contain binlog position to start replicating from
//   out_glev - on return will contain Gtid_list event reflecting the binlog
//       state that was at this position. Has the same meaning as initial
//       Gtid_list event written in the beginning of each binlog file.
//   out_saw_regen - on return will contain value of "regenerated" flag from
//       the index meta info during the time of opening the file
//   errormsg - on return will contain error message in case of error
//
// Returns:
//   false if binlog position was found successfully, returns true otherwise.
//   In case of error *errormsg will contain specific error message.
static bool
gtid_find_binlog_pos(const char *gtid_index_name,
                     IO_CACHE *binlog_cache,
                     Format_description_log_event *fdle,
                     bool is_binlog_active,
                     slave_connection_state *state,
                     ...
                     const char **errormsg)
{
  ...
  rpl_gtid gtid;
  my_off_t binlog_pos;
  Gtid_log_event *binlog_gev= NULL;

  if (gtid_scan_binlog_index(gtid_index_name, binlog_cache, fdle,
                             is_binlog_active, state,
                             slave_gtid_strict_mode, index_meta, &gtid_str,
                             &binlog_pos, out_glev, out_saw_regen, errormsg))
  {
    goto end;
  }
  ...
  gtid_ptr= gtid_str.c_ptr();
  if (gtid_parser_helper(&gtid_ptr, gtid_ptr + gtid_str.length(), &gtid))
  {
    *errormsg= "Cannot parse GTID value in the GTID index";
    sql_print_error("%s: '%s'", *errormsg, gtid_str.c_ptr_safe());
    goto end;
  }

  my_b_seek(binlog_cache, binlog_pos);
  ev= Log_event::read_log_event(binlog_cache, 0, fdle,
                                opt_master_verify_checksum);
  if (!ev || ev->get_type_code() != GTID_EVENT)
  {
    *errormsg= "GTID index and binlog are unsynchronized, cannot read "
               "GTID event from binlog file";
    goto end;
  }
  binlog_gev= static_cast<Gtid_log_event *>(ev);
  if (gtid.domain_id != binlog_gev->domain_id ||
      gtid.server_id != binlog_gev->server_id ||
      gtid.seq_no != binlog_gev->seq_no)
  {
    *errormsg= "GTID index and binlog are unsynchronized, GTID events have "
               "different values";
    sql_print_error("%s: %u-%u-%llu vs %u-%u-%llu", *errormsg,
                    gtid.domain_id, gtid.server_id, gtid.seq_no,
                    binlog_gev->domain_id, binlog_gev->server_id,
                    binlog_gev->seq_no);
    goto end;
  }
...
// Regenerate the GTID index file from scratch
//
// Parameters:
//   index_name - file name for GTID index
//   binlog_cache - open IO cache for the binlog
//   fdle - format description event that's read from the binlog and that can
//       be used to read events from it
//   index_meta - GTID index meta info, used to exclude parallel regeneration
//       by several threads and to determine whether index file was just
//       regenerated by another thread or not
//   errormsg - error message returned in case of error
//
// Returns:
//   false in case of successful regeneration or detection that the file was
//   just regenerated. true in case of error, *errormsg is set to specific
//   error message in this case.
static bool
regenerate_gtid_index(const char *index_name,
                      IO_CACHE *binlog_cache,
                      Format_description_log_event *fdle,
                      GTID_INDEX_META *index_meta,
                      const char **errormsg)
{
  bool ret= true;
  char tmp_index_name[FN_REFLEN];

  mysql_mutex_lock(&index_meta->regen_lock);
  mysql_mutex_lock(&gtid_index_meta_lock);
  if (index_meta->regenerated)
  {
    mysql_mutex_unlock(&gtid_index_meta_lock);
    ret= false;
    goto end;
  ...
  if (!fn_format(tmp_index_name, index_name, mysql_data_home, ".tmp",
                 MY_REPLACE_EXT | MY_SAFE_PATH))
  {
    *errormsg= "Cannot create temporary GTID index file name";
    goto end;
  }
  if (create_full_gtid_index(tmp_index_name, binlog_cache, fdle, errormsg))
    goto end;

  mysql_mutex_lock(&gtid_index_meta_lock);
  if (my_rename(tmp_index_name, index_name, MYF(MY_WME)))
  {
    mysql_mutex_unlock(&gtid_index_meta_lock);
    *errormsg= "Cannot rename temporary GTID index file into permanent one.";
    goto end;
  }
  index_meta->regenerated= true;
  ret= false;
  mysql_mutex_unlock(&gtid_index_meta_lock);

end:
  mysql_mutex_unlock(&index_meta->regen_lock);
  return ret;
}
...
gtid_find_binlog_file(slave_connection_state *state,
                      ...
                      char *out_name, my_off_t *out_pos,
                      slave_connection_state *until_gtid_state,
                      bool slave_gtid_strict_mode)
{
  MEM_ROOT memroot;
  ...
  Gtid_list_log_event *glev= NULL;
  Gtid_list_log_event *index_glev= NULL;
  Format_description_log_event *fdle= NULL;
  int error= 0;
  char buf[FN_REFLEN];
  File file= -1;
  IO_CACHE cache;
  char gtid_index_name[FN_REFLEN];
  bool saw_regen= false;
  bool is_active;
  GTID_INDEX_META *index_meta= NULL;
  ...
  // List of binlog files is returned in the opposite order, i.e. the first file
  // will be the active one, then the previous one, and so on.
  is_active= true;
  while (list)
  {
    if (!list->next)
    ...
    *errormsg= get_gtid_list_event(&cache, &glev, &fdle);
    if (*errormsg)
    {
      error= ER_MASTER_FATAL_ERROR_READING_BINLOG;
      goto end;
  ...
  if (gen_gtid_index_file_name(buf, gtid_index_name))
  {
    *errormsg= "Cannot generate GTID index file name.";
    error= ER_MASTER_FATAL_ERROR_READING_BINLOG;
    goto end;
  }
  error= gtid_find_binlog_pos(gtid_index_name, &cache, fdle, is_active, state,
                              slave_gtid_strict_mode, &index_meta,
                              out_pos, &index_glev, &saw_regen, errormsg);
  if (error && !saw_regen && !is_active)
  {
    sql_print_error("Cannot find binlog position in GTID index file: %s. "
                    "Regenerating the index.", *errormsg);
    *errormsg= NULL;
    if (regenerate_gtid_index(gtid_index_name, &cache, fdle, index_meta,
                              errormsg))
    {
      sql_print_error("Cannot generate GTID index file: %s.", *errormsg);
      error= ER_MASTER_FATAL_ERROR_READING_BINLOG;
    }
    else
    {
      error= gtid_find_binlog_pos(gtid_index_name, &cache, fdle, false, state,
                                  slave_gtid_strict_mode, &index_meta,
                                  out_pos, &index_glev, &saw_regen, errormsg);
      DBUG_ASSERT(saw_regen);
    }
  }
  if (error)
  {
    ...
    {
      sql_print_error("Cannot find binlog position in GTID index file '%s' "
                      "even after regeneration: %s.",
                      gtid_index_name, *errormsg);
    }
    else
    {
      DBUG_ASSERT(is_active);
      sql_print_error("Cannot find binlog position in the currently active "
                      "GTID index file '%s': %s.",
                      gtid_index_name, *errormsg);
    }
    error= ER_MASTER_FATAL_ERROR_READING_BINLOG;
  }
  release_gtid_index_meta_ref(index_meta);
		       ...
		       ushort flags)
{
  ...
  */
  ...
  if (info.using_gtid_state)
  {
    ...
    if ((error= gtid_find_binlog_file(&info.gtid_state, &errmsg, &error_gtid,
                                      search_file_name, &pos,
                                      info.until_gtid_state,
                                      info.slave_gtid_strict_mode)))
    {
      my_errno= error;
      goto err;
    }
...
rpl_init_gtid_slave_state()
{
  ...
  // This is a hijacking of somewhat unrelated function, but all I need is to
  // initialize somewhere at the start of mysqld.
  mysql_mutex_init(NULL, &gtid_index_meta_lock, NULL);
  my_hash_init(&gtid_indexes, system_charset_info, 10, 0, 0,
               (my_hash_get_key) &get_index_name_from_meta,
               &free_gtid_index_meta, 0);
}
...
rpl_deinit_gtid_slave_state()
{
  ...
  // This is a hijacking of somewhat unrelated function, but all I need is to
  // de-initialize somewhere at the end of mysqld.
  mysql_mutex_destroy(&gtid_index_meta_lock);
  my_hash_free(&gtid_indexes);
}
  ...
  while ((kill_thd= it++))
  {
    ...
    */
        ...
        !pthread_equal(kill_thd->real_id, thd->real_id))
    {
      ...
      mysql_mutex_lock(&kill_thd->LOCK_thd_data);
      // ask the thread to die
      kill_thd->awake(KILL_CONNECTION);
      mysql_mutex_unlock(&kill_thd->LOCK_thd_data);
    }
  }
  mysql_mutex_unlock(&LOCK_thread_count);
}
