...
class StatsServerConnectState {
 public:
  StatsServerConnectState() : tier(0), searchTier(0), startIndex(-1),
                              currentOffset(-1), hasRetried(false),
                              foundMatch(false) { }
...
private:
  ...
  struct ServerInfo {
    ...
    ServerInfo(const char* n, int p)
        : name(n), lastAccessed(0), port(p), connectTry(0),
          connectFail(0), isAlive(true) { }
    ServerInfo()
        : lastAccessed(0), port(0), connectTry(0),
          connectFail(0), isAlive(false) { }
  ...
  struct ServerTblRecord {
    ...
    ServerTblRecord(const std::string& s, const std::string& d,
                    const std::string& t, int p, int tr)
        : serverName(s), dbName(d), tblName(t), port(p), tier(tr) {}
  };
