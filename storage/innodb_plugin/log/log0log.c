	...
	log_group_t*	group)	/*!< in: log group or a dummy pointer */
{
	...
	if ((ulint)group & 0x1UL) {
		...
		if (srv_unix_file_flush_method != SRV_UNIX_O_DSYNC
			...
			fil_flush(group->space_id, FLUSH_TYPE_LOG_IO_COMPLETE);
		}
...
# endif
	...
	if (srv_unix_file_flush_method == SRV_UNIX_O_DSYNC) {
		...
		fil_flush(group->space_id, FLUSH_TYPE_LOG_WRITE_UP_TO);
		log_sys->flushed_to_disk_lsn = log_sys->write_lsn;
		log_sys->n_syncs++;
		log_sys->log_sync_syncers[caller]++;
	}
				...
				made to log files */
{
	...
	if (srv_unix_file_flush_method != SRV_UNIX_NOSYNC) {
          fil_flush_file_spaces(FIL_TABLESPACE, FLUSH_TYPE_CHECKPOINT);
	}
...
/*=========================*/
{
	...
	fil_flush(group->archive_space_id, FLUSH_TYPE_ARCHIVE);
...
loop:
	...
	fil_flush_file_spaces(FIL_LOG, FLUSH_TYPE_OTHER);
