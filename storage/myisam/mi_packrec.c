...
static uchar get_bit_buff_byte(MI_BIT_BUFF *bit_buff, int byte_num)
{
  return bit_buff->end - bit_buff->pos > byte_num ? bit_buff->pos[byte_num] : 0;
}
			 ...
			 uchar *end)
{
  ...
  do
  {
	...
	((((uint) get_bit_buff_byte(bit_buff, 3))) +
	 (((uint) get_bit_buff_byte(bit_buff, 2)) << 8) +
	 (((uint) get_bit_buff_byte(bit_buff, 1)) << 16) +
	 (((uint) get_bit_buff_byte(bit_buff, 0)) << 24));
      bit_buff->pos+=4;
      bits+=32;
    }
    /*
			 ...
			 uchar *end)
{
  ...
  do
  {
	...
	(((uint) get_bit_buff_byte(bit_buff, 2))) +
	  (((uint) get_bit_buff_byte(bit_buff, 1)) << 8) +
	    (((uint) get_bit_buff_byte(bit_buff, 0)) << 16);
      bit_buff->pos+=3;
      bits+=24;
#else
	...
	bit_buff->current_byte=  (bit_buff->current_byte << 8) +
	  (uint) get_bit_buff_byte(bit_buff, 0);
	bit_buff->pos++;
	bits+=8;
      }
      else
      {
	bit_buff->current_byte= ((uint) get_bit_buff_byte(bit_buff, 0) << 8) +
	  ((uint) get_bit_buff_byte(bit_buff, 1));
	bit_buff->pos+=2;
	bits+=16;
      }
#endif
...
#if BITS_SAVED == 64
  bit_buff->current_byte=  ((((uint) get_bit_buff_byte(bit_buff, 7))) +
			     (((uint) get_bit_buff_byte(bit_buff, 6)) << 8) +
			     (((uint) get_bit_buff_byte(bit_buff, 5)) << 16) +
			     (((uint) get_bit_buff_byte(bit_buff, 4)) << 24) +
			     ((ulonglong)
			      ((((uint) get_bit_buff_byte(bit_buff, 3))) +
			       (((uint) get_bit_buff_byte(bit_buff, 2)) << 8) +
			       (((uint) get_bit_buff_byte(bit_buff, 1)) << 16) +
			       (((uint) get_bit_buff_byte(bit_buff, 0)) << 24)) << 32));
  bit_buff->pos+=8;
#else
#if BITS_SAVED == 32
  bit_buff->current_byte=  (((uint) get_bit_buff_byte(bit_buff, 3)) +
			     (((uint) get_bit_buff_byte(bit_buff, 2)) << 8) +
			     (((uint) get_bit_buff_byte(bit_buff, 1)) << 16) +
			     (((uint) get_bit_buff_byte(bit_buff, 0)) << 24));
  bit_buff->pos+=4;
#else
  bit_buff->current_byte=  (uint) (((uint) get_bit_buff_byte(bit_buff, 1))+
				    (((uint) get_bit_buff_byte(bit_buff, 0)) << 8));
  bit_buff->pos+=2;
#endif
#endif
}
